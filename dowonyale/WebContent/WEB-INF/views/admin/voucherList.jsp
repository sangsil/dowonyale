...
<script>
	$(function(){
		$( "#search_type_set, #ivoucher_sex_set, #uvoucher_sex_set" ).buttonset();
		...
		//숫자만입력
		$(document).on("keyup", "#ivoucher_amt,#isuyeop_cnt,#isuyeop_amt,#uvoucher_amt,#usuyeop_cnt,#usuyeop_amt", function() {$(this).val( $(this).val().replace(/[^0-9]/gi,"") );});
		$(document).on("blur", "#ivoucher_amt,#isuyeop_cnt,#isuyeop_amt,#uvoucher_amt,#usuyeop_cnt,#usuyeop_amt", function() {$(this).val( $(this).val().replace(/[^0-9]/gi,"") );});	
// 		//달력
// 		$("#icharge_month").datepicker({
...
// 		});
		...
		$("#btn_del").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/voucherDelete.dowon?arr_voucher_id="+get_chked_values(),
					type : "post",
					dataType: "json",
		...
		$("#btn_del_cancel").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/voucherDeleteCancel.dowon?arr_voucher_id="+get_chked_values(),
					type : "post",
					dataType: "json",
					success: function(result){
						alert("삭제취소 되었습니다.");
		...
		$("#btn_excel").bind("click",function(){
			if($("#startNum").val() == null || $("#startNum").val() == ''){$("#startNum").val(0);}
			$("form[name=frm]").attr("action","/admin/voucherListExcel.dowon");
			$("form[name=frm]").submit();			
		});
		/*//엑셀 */
	...
	/*	등록	*/
		...
		$("#insert_user_modal").dialog({
			...
			buttons: {
				"등록": function(){
// 					if($("#icharge_month").val().length < 1) {alert("년월을 확인해 주세요"); $("#icharge_month").focus(); return false;}
					...
					if($("#ivoucher_amt").val().length < 1) {alert("바우처지원금을 확인해 주세요"); $("#ivoucher_amt").focus(); return false;}
					if($("#isuyeop_cnt").val().length < 1) {alert("수업구분을 확인해 주세요"); $("#isuyeop_cnt").focus(); return false;}
					if($("#isuyeop_amt").val().length < 1) {alert("수업부담금을 확인해 주세요"); $("#isuyeop_amt").focus(); return false;}
					
					if(confirm("등록 하시겠습니까?"))
					{
						$("#insertModalForm").ajaxSubmit({
							url : "/admin/voucherInsert.dowon",
							type : "post",
							dataType: "json",
							success: function(result){
								alert("등록 되었습니다.");
	...
	//수정
		$(".modCell").bind("click",function(){
			...
			$("#uvoucher_id").val(substr[0]);
// 			$("#ucharge_month").text(substr[1]);
			$("#ugwamok").text(substr[1]);
			$("#usuyeop_gubun").text(substr[2]);
			$("#uvoucher_amt").val(substr[3]);
			$("#usuyeop_cnt").val(substr[4]);
			$("#usuyeop_amt").val(substr[5]);
		...
		$("#update_user_modal").dialog({
			...
			buttons: {
				"수정": function(){
					if($("#uvoucher_amt").val().length < 1) {alert("바우처지원금을 확인해 주세요"); $("#uvoucher_amt").focus(); return false;}
					if($("#usuyeop_cnt").val().length < 1) {alert("수업구분을 확인해 주세요"); $("#usuyeop_cnt").focus(); return false;}
					if($("#usuyeop_amt").val().length < 1) {alert("수업부담금을 확인해 주세요"); $("#usuyeop_amt").focus(); return false;}
					
					if(confirm("수정 하시겠습니까?"))
					{
						$("#updateModalForm").ajaxSubmit({
							url : "/admin/voucherUpdate.dowon",
							type : "post",
							dataType: "json",
							success: function(result){
								alert("수정 되었습니다.");
...
<div class="contents">
	<!-- con1 -->
    <div class="con1">바우처관리</div>
   	<!-- //con1 -->
	
    <!-- con2 -->
    <div class="con2">
		<form id="frm" name="frm" method="post" action="/admin/voucherList.dowon">
			...
			<c:set var="default_url" value="/admin/voucherList.dowon"></c:set>
        ...
        <table class="dataTable" border="1" cellspacing="0" cellpadding="3">
            <thead>
                  <tr>
                    ...
                    <th class="col60">바우처지원금</th>
                    <th class="col50">수업횟수</th>    
                    <th class="col50">수업부담금</th>
			...
			<c:if test="${empty resultList}">
				<tr><td colspan="12">등록된 바우처내역이 없습니다.</td></tr>
			</c:if>
			<c:if test="${not empty resultList}">
				<c:forEach var="List" items="${ resultList }">
					<tr class="listCell">
						<td><input type="checkbox" name="checkB" id="${List.voucher_id }"/></td>
						<td class="modCell btn" title="${List.voucher_id }&&${List.gwamok_name }&&${List.suyeop_gubun_name }&&${List.voucher_amt }&&${List.suyeop_cnt }&&${List.suyeop_amt }">
						...
						<td><c:out value="${List.suyeop_amt }"/></td>
						<td>
...
<!-- insert -->
<div id="insert_user_modal" title="바우처등록">
...
<!-- 				<td>년월</td> -->
<!-- 				<td> -->
<!-- 					<input type="text" name="charge_month" id="icharge_month" size="20" maxlength="20" class="text ui-widget-content ui-corner-all" readonly/> -->
<!-- 				</td> -->
<!-- 			</tr> -->
			<tr>
				<td>과목</td>
			...
			<tr>				
				<td>바우처지원금</td>
				<td>
					<input type="text" name="voucher_amt" id="ivoucher_amt" size="6" maxlength="6" class="text ui-widget-content ui-corner-all"/>		
				</td>
			</tr>
			<tr>
				<td>수업횟수</td>
				<td>
					<input type="text" name="suyeop_cnt" id="isuyeop_cnt" size="6" maxlength="3" class="text ui-widget-content ui-corner-all"/>
				</td>
			</tr>
			<tr>
				<td>수업부담금</td>
				<td>
					<input type="text" name="suyeop_amt" id="isuyeop_amt" size="6" maxlength="6" class="text ui-widget-content ui-corner-all"/>
				</td>
			</tr>
        </table>
	</form>
...
<!-- update -->
<div id="update_user_modal" title="바우처수정">
	<form name="updateModalForm" id="updateModalForm" method="post">
        <table class="modalTable_user" id="modalTable_user" border="1" cellspacing="0" cellpadding="3">
<!-- 			<tr> -->
<!-- 				<td>년월</td> -->
<!-- 				<td> -->
<!-- 					<input type="hidden" name="voucher_id" id="uvoucher_id" /> -->
<!-- 					<span id="ucharge_month" /> -->
<!-- 				</td> -->
<!-- 			</tr> -->
			<tr>
				<td>과목</td>
			...
			<tr>				
				<td>바우처지원금</td>
				<td>
					<input type="text" name="voucher_amt" id="uvoucher_amt" size="6" maxlength="6" class="text ui-widget-content ui-corner-all"/>		
				</td>
			</tr>
			<tr>
				<td>수업횟수</td>
				<td>
					<input type="text" name="suyeop_cnt" id="usuyeop_cnt" size="6" maxlength="3" class="text ui-widget-content ui-corner-all"/>
				</td>
			</tr>
			<tr>
				<td>수업부담금</td>
				<td>
					<input type="text" name="suyeop_amt" id="usuyeop_amt" size="6" maxlength="6" class="text ui-widget-content ui-corner-all"/>
				</td>
			</tr>
        </table>
	</form>
