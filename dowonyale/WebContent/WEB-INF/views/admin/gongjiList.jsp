...
<script>
	$(function(){
		...
		$("#btn_del").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/gongjiDelete.dowon?arr_board_id="+get_chked_values(),
					type : "post",
					dataType: "json",
		...
		$("#btn_del_cancel").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/gongjiDeleteCancel.dowon?arr_board_id="+get_chked_values(),
					type : "post",
					dataType: "json",
					success: function(result){
						alert("삭제 취소 되었습니다.");
		...
		$("#btn_excel").bind("click",function(){
			if($("#startNum").val() == null || $("#startNum").val() == ''){$("#startNum").val(0);}
			document.frm.action = "/admin/gongjiListExcel.dowon";
			document.frm.submit();
		});
		/*//엑셀 */
		...
		$("#btn_reg").click(function(e) {
			$("#board_id").val("");
			$("form[name=frm]").attr("action", "/admin/gongjiInUpForm.dowon");
			$("form[name=frm]").submit();
		});
...
<div class="contents">
    ...
    <!-- con2 -->
    <div class="con2">
		<form id="frm" name="frm" method="post" action="/admin/gongjiList.dowon">
			...
			<c:set var="default_url" value="/admin/gongjiList.dowon"></c:set>
