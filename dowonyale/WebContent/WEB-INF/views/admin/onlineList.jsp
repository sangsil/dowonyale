...
<script>
	$(function(){
		...
		$("#btn_del").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/onlineDelete.dowon?arr_board_id="+get_chked_values(),
					type : "post",
					dataType: "json",
		...
		$("#btn_del_cancel").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/onlineDeleteCancel.dowon?arr_board_id="+get_chked_values(),
					type : "post",
					dataType: "json",
					success: function(result){
						alert("삭제 취소 되었습니다.");
		...
		$("#btn_excel").bind("click",function(){
			if($("#startNum").val() == null || $("#startNum").val() == ''){$("#startNum").val(0);}
			document.frm.action = "/admin/onlineListExcel.dowon";
			document.frm.submit();
		});
		/*//엑셀 */
		...
		$("#btn_reg").click(function(e) {
			$("#board_id").val("");
			$("form[name=frm]").attr("action", "/admin/onlineInUpForm.dowon");
			$("form[name=frm]").submit();
		});
...
<div class="contents">
	<!-- con1 -->
    <div class="con1">온라인상담관리</div>
   	<!-- //con1 -->
	
    <!-- con2 -->
    <div class="con2">
		<form id="frm" name="frm" method="post" action="/admin/onlineList.dowon">
			...
			<c:set var="default_url" value="/admin/onlineList.dowon"></c:set>
			...
			<c:if test="${not empty resultList}">
				<c:forEach var="List" items="${ resultList }">
					<tr class="listCell" id="${List.board_id }">
						...
						<td>
							<c:if test="${List.reply_yn eq 'N'}"></c:if>
							<c:if test="${List.reply_yn eq 'Y'}">답변완료</c:if>
						</td>						
						<td>
