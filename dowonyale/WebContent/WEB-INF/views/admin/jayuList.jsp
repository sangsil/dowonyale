...
<script>
	$(function(){
		...
		$("#btn_del").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/jayuDelete.dowon?arr_board_id="+get_chked_values(),
					type : "post",
					dataType: "json",
		...
		$("#btn_del_cancel").bind("click",function(){
			...
			{
				$("#frm").ajaxSubmit({
					url : "/admin/jayuDeleteCancel.dowon?arr_board_id="+get_chked_values(),
					type : "post",
					dataType: "json",
					success: function(result){
						alert("삭제 취소 되었습니다.");
		...
		$("#btn_excel").bind("click",function(){
			if($("#startNum").val() == null || $("#startNum").val() == ''){$("#startNum").val(0);}
			document.frm.action = "/admin/jayuListExcel.dowon";
			document.frm.submit();
		});
		/*//엑셀 */
		...
		$("#btn_reg").click(function(e) {
			$("#board_id").val("");
			$("form[name=frm]").attr("action", "/admin/jayuInUpForm.dowon");
			$("form[name=frm]").submit();
		});
...
<div class="contents">
	<!-- con1 -->
    <div class="con1">자유게시판관리</div>
   	<!-- //con1 -->
	
    <!-- con2 -->
    <div class="con2">
		<form id="frm" name="frm" method="post" action="/admin/jayuList.dowon">
			...
			<c:set var="default_url" value="/admin/jayuList.dowon"></c:set>
