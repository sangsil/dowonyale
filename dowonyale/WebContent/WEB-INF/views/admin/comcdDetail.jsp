			...
			if(confirm("삭제 하시겠습니까?"))
			{
				$("#frm").ajaxSubmit({
					...
					success: function(result){
						...
						$("input[name=cd_id]").val("${result.cd_id }");
						$("form[name=frm]").attr("action","${default_url}");
						$("form[name=frm]").submit();
					},
...
<div class="contents">
    ...
    <div class="con1_1">
		...
		<table border="1" cellspacing="0" cellpadding="3">
            ...
            <tbody>
				<tr>
					<td>
						<c:out value="${result.cd_id }"/>
					</td>
					<td>
						<c:out value="${result.cd_name }"/>
					</td>
					<td>
					...
					<td>
						<fmt:parseDate var="dateTempParse" value="${result.up_date }" pattern="yyyy-MM-dd HH:mm:ss.S"/>
						<fmt:formatDate value="${dateTempParse }" pattern="yyyy-MM-dd"/>
					</td>
					<td><c:out value="${result.up_user_name }"/></td>
					<td>
    ...
    <div class="con2">
		<form id="frm" name="frm" method="post" action="/admin/comcdList.dowon">
			...
			<c:set var="default_url" value="/admin/comcdDetail.dowon"></c:set>
...
<div id="insert_user_modal" title="코드등록">
	<form name="insertModalForm" id="insertModalForm" method="post">
		<input type="hidden" name="high_cd_id" value="${result.cd_id }" />
